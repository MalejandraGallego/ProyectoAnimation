...
// // Tarea: Modificar el programa de saludo para recibir letras.

// Se cambia consoleke a consolekeyinfo para poder accder a k.kychar  este acepta cualquier caracter
let procesarTecladoSaludo (k: ConsoleKeyInfo) estado =
    match k.Key with 
    | ConsoleKey.Enter ->
        { estado with EstadoDeSaludo = MostrandoSaludo; RedibujarPantalla = true }
    | ConsoleKey.Backspace ->
        if estado.Buffer.Length > 0 then
            { estado with 
                Buffer = estado.Buffer.Substring(0, estado.Buffer.Length - 1)
                RedibujarPantalla = true }
        else
            estado
    | _ ->
        if not (Char.IsControl k.KeyChar) then
            { estado with 
                Buffer = estado.Buffer + string k.KeyChar
                RedibujarPantalla = true }
        else
            estado
...
// |> procesarTecladoSaludo k.Key  se cambia a |> procesarTecladoSaludo k 
//Pasa el ConsoleKeyInfo completo para que la función pueda leer k.KeyChar
//
let procesarTeclado estado =
    if Console.KeyAvailable then 
        let k = Console.ReadKey true
        estado 
        |> procesarTecladoReloj k.Key
        |> procesarTecladoSaludo k    // <-- k completo, no k.Key
    else
        estado
