...
type State = {
    ...
    EnemigoY: int
    EnemigoDir: int
    MisilesEnemigo: Misil list  // se agrega para misiles enemigos
}
...
let estadoInicial = {
    ...
    EnemigoY= 0
    EnemigoDir=1
    MisilesEnemigo = [] // se agrega para misiles enemigo
}
...
//Se agrega para misiles del enemigo

let actualizarMisilesEnemigo state =  
    
    let misilesMovidos =
        state.MisilesEnemigo
        |> List.map (fun m -> {m with X = m.X - 1})
        |> List.filter (fun m -> m.X > 0)

    let misilesConDisparo =
        if state.Tick % 20 = 0 then
            let nuevoMisil = { X = state.EnemigoX - 1; Y = state.EnemigoY }
            nuevoMisil :: misilesMovidos
        else
            misilesMovidos

    if misilesConDisparo <> state.MisilesEnemigo then
        {state with MisilesEnemigo = misilesConDisparo; RedibujarPantalla = true}
    else
        state
...
// se agrega para dibujar los misiles del enemigo

let redibujarMisilesEnemigo state =
    state.MisilesEnemigo
    |> List.iter (fun misil ->
...
let redibujarPantalla state =
    if state.RedibujarPantalla then 
        ...
        [|
            redibujarMisiles
            redibujarMisilesEnemigo  // agregamos para redibujar misiles enemigo
            redibujarAlien
            redibujarEnemigo
...
let rec mainLoop state =
    let newState =
        ...
        |> actualizarTick
        |> actualizarMisiles
        |> actualizarMisilesEnemigo // se agrega actualziar misilesenemigo
        |> actualizarEnemigo
        |> procesarTeclado
        |> redibujarPantalla
